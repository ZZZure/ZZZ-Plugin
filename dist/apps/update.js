...
export class update extends plugin {
    ...
    async checkUpdateTask() {
        ...
        if (result.hasUpdate) {
            ...
            if (!logs?.length)
                return;
            if (logs[0].commit === updateInfo.lastCheckCommit)
                return;
            const botInfo = { nickname: 'ZZZ-Plugin更新', user_id: Bot.uin };
            const msgs = [
                {
                    message: [`[${pluginName}]有${logs.length || 1}个更新`],
                    ...botInfo
                }
            ];
            ...
            try {
                ForMsg.data = ForMsg.data
                    ...
                    .replace(/___+/, '<title color="#777777" size="26">ZZZ-Plugin更新</title>');
            }
            catch (err) {
            }
            const masters = config.masterQQ;
            for (const master of masters) {
                if (master.toString() === 'stdin')
                    continue;
            ...
            updateInfo.lastCheckCommit = logs[0].commit;
        }
    }
}
