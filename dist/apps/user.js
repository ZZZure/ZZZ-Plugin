...
export class User extends ZZZPlugin {
    ...
    async bindDevice() {
        ...
        await this.reply(`为UID ${uid}绑定设备，请发送设备信息(建议私聊发送)，或者发送“取消”取消绑定`, false, { at: true, recallMsg: getRecallMsg() });
    }
    async toBindDevice() {
    ...
    async bindDeviceHelp() {
        const msgs = [
            ...
            _.get(settings.getConfig('config'), 'url', 'https://ghproxy.mihomo.me/https://raw.githubusercontent.com/forchannot/get_device_info/main/app/build/outputs/apk/debug/app-debug.apk'),
            '2. 打开后点击按钮复制',
            '3. 给机器人发送"%绑定设备"指令',
            '4. 机器人会提示发送设备信息',
            '5. 粘贴设备信息发送',
