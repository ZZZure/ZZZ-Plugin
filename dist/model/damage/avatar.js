    ...
    try {
        ...
        const loadCalcJS = async () => {
            if (!fs.existsSync(calcFilePath))
                return;
            const m = await import(`${prefix}/character/${charName}/${calcFile}?${Date.now()}`);
            if (m.default) {
        ...
        const loadScoreJS = async () => {
            if (!fs.existsSync(scoreFilePath))
                return;
            const m = await import(`${prefix}/character/${charName}/${scoreFile}?${Date.now()}`);
            const fnc = m.default;
...
async function importFile(type, name, isWatch = false) {
    ...
    try {
        ...
        const m = await import(`${prefix}/${type}/${name}${isUser ? '_user' : ''}.js?${Date.now()}`);
        if (!m.calc && !m.buffs)
