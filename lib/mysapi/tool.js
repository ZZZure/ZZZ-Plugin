...
export default class ZZZApiTool {
  ...
  getUrlMap = (data = {}) => {
    const {
      productName = settings.getConfig('device')?.productName || 'J9110',
      deviceType = settings.getConfig('device')?.productType || 'J9110',
      modelName = settings.getConfig('device')?.modelName || 'Sony',
      oaid = settings.getConfig('device')?.oaid || 'e3ba65d61709d17074a306072a7c3a2ec70e6d374ca6d29eb6b1b965cbb68e23',
      deviceInfo = settings.getConfig('device')?.deviceInfo || 'Sony/J9110/J9110:11/55.2.A.4.332/055002A004033203408384484:user/release-keys',
      board = settings.getConfig('device')?.board || 'msmnile',
    } = data;
