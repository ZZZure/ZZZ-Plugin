...
export async function getCommitLog(e: EventType) {
  // @ts-expect-error
  e ||= this.e
...
export async function hasUpdate(e: EventType) {
  // @ts-expect-error
  e ||= this.e
  if (!ZZZUpdate) return false
  const updatePlugin = new ZZZUpdate()
...
/** 开启/关闭自动更新推送 */
export async function enableAutoUpdatePush(e: EventType) {
  // @ts-expect-error
  e ||= this.e
  if (!e.isMaster) {
...
/** 设置自动更新时间 */
export async function setCheckUpdateCron(e: EventType) {
  // @ts-expect-error
  e ||= this.e
  if (!e.isMaster) {
    return e.reply('仅限主人设置', false, { at: true, recallMsg: 100 })
