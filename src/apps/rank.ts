...
export class Rank extends ZZZPlugin {
  ...
  constructor() {
    super({
      ...
      rule: [
        ...
        {
          reg: `${rulePrefix}(绝境|(危局|强袭|强袭战|危局强袭战)绝境|绝境(危局|强袭|强袭战|危局强袭战))排名$`,
          fnc: 'deadlyHardRank'
        },
        {
  ...
  async deadlyHardRank() {
    ...
    let scoredData = filteredByUser
      ...
      .map(item => {
        const hardList = _.get(item, 'result.hard_list', []) as Array<{ score?: number; star?: number; total_star?: number; challenge_time?: { year: number; month: number; day: number; hour: number; minute: number; second: number } }>
