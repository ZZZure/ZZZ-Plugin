  ...
  async skills() {
    ...
    const levels = levelsChar
      ? levelsChar.split(/\.|\s+/).map(x => {
        ...
        if (_.isString(trimmed) && trimmed.length > 0) return trimmed.toUpperCase().charCodeAt(0) - 64
        return undefined
      })
      : []
    const parsedLevels = levels as Array<number | undefined>
    const [
      BasicLevel = 12,
      DodgeLevel = 12,
      AssistLevel = 12,
