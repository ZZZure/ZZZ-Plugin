...
async function getLiveData(actId: string) {
...
async function getCode(version: string, actId: string) {
  const ret = await getData('code', { version, actId })
  if (ret.error || ret.retcode !== 0) {
    return { error: ret.error || '兑换码数据异常' }
  }

  return ret.data.code_list.map((codeInfo: any) => ({
...
export async function getCodeMsg() {
  ...
  if ('error' in liveData) return `获取兑换码失败：\n${liveData.error}`
