...
export const updateGachaLog = async (authKey: string, uid: string, region?: string, game_biz?: string) => {
  ...
  // 遍历所有池子
  for (const name of Object.keys(gacha_type_meta_data) as GachaType[]) {
    if (!previousLog[name]) {
      // 如果没有数据，初始化为空数组
      previousLog[name] = []
    }
    ...
    // 获取上一次保存的数据
    const lastSaved = previousLog[name]?.[0] as SingleGachaLog | undefined
    // 初始化页数和最后一个数据的 id
    let page = 1
    let endId = '0'
    // 新数据
...
export const anaylizeGachaLog = async (uid: string) => {
  ...
  const result = []
  // 遍历所有池子
  for (const name of Object.keys(savedData) as GachaType[]) {
    // 转换为 SingleGachaLog 对象
