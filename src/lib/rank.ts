...
export function setGroupRankAllowed(allowed: boolean | number) {
  settings.setSingleConfig('rank', 'allow_group', !!allowed)
}

export function isGroupRankAllowed(): boolean {
...
export async function setUserRankAllowed(rank_type: string, uid: string, group_id: string | number, allowed: number) {
  await redis.set(`ZZZ:RANK:${rank_type}:${group_id}:${uid}`, Number(allowed))
}

export async function addUserToGroupRank(rank_type: string, uid: string, group_id: string | number) {
  await setUserRankAllowed(rank_type, uid, group_id, DEFAULT_RANK_ALLOWED)
}

export async function isUserRankAllowed(rank_type: string, uid: string, group_id: string | number): Promise<number> {
...
export async function removeUserRankRecord(rank_type: string, group_id: string | number, uid: string) {
  await redis.del(`ZZZ:RANK:${rank_type}:${group_id}:${uid}`)
}

export async function setUidAndQQ(group_id: string | number, uid: string, qq: string | number) {
  await redis.set(`ZZZ:RANK:UID2QQS:${group_id}:${uid}:${qq}`, 1)
}

export async function removeUidFromUid2QQ(group_id: string | number, uid: string) {
...
export async function getUid2QQsMapping(group_id: string | number): Promise<Record<string, string[]>> {
  const keys = await redis.keys(`ZZZ:RANK:UID2QQS:${group_id}:*`)
  const uid2qqs: Record<string, string[]> = {}
...
export async function removeUid2QQsMapping(group_id: string | number) {
  const keys = await redis.keys(`ZZZ:RANK:UID2QQS:${group_id}:*`)
  if (keys.length > 0) {
    await redis.del(keys)
  }
