...
伤害计算模块提供了注册、管理各buff的接口[BuffManager](./BuffManager.ts#L153)，所有buff都需要通过此类的实例**buffM**进行注册、管理
...
- buff注册方法：武器、套装、角色每部分buff的注册，都存在两种方式：
  ...
  - 二：[函数导出](./character/模板/calc.js#L7)：调用[BuffManager](./BuffManager.ts#L153)实例**buffM**的**new**方法，传入你需要注册的buff即可
