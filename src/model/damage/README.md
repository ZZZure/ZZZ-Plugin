...
> 底层已对伤害计算进行了规范化、模块化，并提供类型支持优化开发体验，只需要填参数就可以实现常见的伤害计算逻辑，即使不懂代码也可以参考模板独立完成。若存在问题、建议，可于插件群内询问或与我沟通

后文将详细讲解插件伤害计算逻辑，建议完整阅读：
...
局内Buff，即仅在局内生效的所有增益，一个buff可以同时作用于多种属性和多个技能。合理管控buff，是做好伤害计算**最重要**的一步

### 认识buff

每个buff由各项[buff参数](./BuffManager.ts#L60)组成，重要参数：
...
```js
{
   ...
   * @number
   * - 一般情况下此值即为提高值
   * - 当buff增益类型为生命值/防御力/攻击力/冲击力/异常掌控时，若此值<1，则将此值默认理解为基础属性的百分比提高
   * @string
...
- **source**：Buff来源。用于管理buff、简化参数、判断生效条件等。查看[buff来源](./BuffManager.ts#L28)

- **type**：Buff增益的类型。查看[增益类型](./BuffManager.ts#L30)
...
完整参数详见[buff参数](./BuffManager.ts#L60)

### 注册buff

伤害计算模块提供了注册、管理各buff的接口[BuffManager](./BuffManager.ts#L160)，所有buff都需要通过此类的实例**buffM**进行注册、管理
...
- buff注册方法：武器、套装、角色每部分buff的注册，都存在两种方式：

  - 一：[直接导出](./character/模板/calc.js#L68)：在计算文件中导出buffs数组，数组中保存各buff即可
  - 二：[函数导出](./character/模板/calc.js#L7)：调用[BuffManager](./BuffManager.ts#L160)实例**buffM**的**new**方法，传入你需要注册的buff即可
...
[点此查看](./character/艾莲/calc.js#L23)艾莲实际伤害计算文件

注意事项：

- 属性异常中**强击**和**碎冰**没有持续时间的概念，总倍率不受持续时间的影响也无法结算紊乱。因此对于作用于**异常持续时间**的buff，其buff.range应填写异常对应的**状态异常**（**畏缩**和**霜寒**），灼烧等既是伤害异常也是状态异常则无需区分

- 对于`“X"(造成的伤害)被视为“Y”(伤害)`此类特殊技能，需要指定技能**重定向参数**，同时上述buff覆盖规则会发生变化，具体请参考[源码内描述](./Calculator.ts#L38)
...
1. 参考[模板](./character/模板/calc.js#L116)，填入各技能相应参数（一般只需要填name、type参数）
...
7. 检查角色所使用武器的计算文件是否存在，若缺失还应编写对应的[武器计算文件](#武器buff)

8. 检查角色所使用套装的计算文件是否存在，若缺失还应编写对应的[套装计算文件](#套装buff)
